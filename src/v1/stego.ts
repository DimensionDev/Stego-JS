...
import { mergeBits, str2bits, setBit, getBit, bits2str, Bit } from './bit.js'
...
export async function encodeImg(
  ...
  const bits = mergeBits(
    randomBits(randomSource, exhaustPixels ? sizeOfBlocks : textBits.length + 8 * copies),
    textBits,
    Array(8 * copies).fill(1),
  )
